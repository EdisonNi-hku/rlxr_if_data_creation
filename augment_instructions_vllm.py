#!/usr/bin/env python3
"""Augment instructions with constraints using a vLLM OpenAI-compatible API."""
...
def build_user_prompt(template: str, raw_prompt: str) -> str:
    return template.replace("{raw_prompt}", raw_prompt)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Augment instructions with constraints using a vLLM model."
    )
    parser.add_argument(
        "--input_dataset",
        required=True,
    ...
    parser.add_argument(
        "--system_prompt_path",
        default="prompt/constraint_augmentation.txt",
        help="Path to the system prompt file.",
    )
    parser.add_argument(
        "--user_prompt_path",
        default="prompt/constraint_augmentation_user.txt",
    ...
    def build_messages(index: int, example: dict) -> Optional[tuple[str, list[dict]]]:
        """Build the messages for an example. Returns (raw_prompt, messages) or None."""
