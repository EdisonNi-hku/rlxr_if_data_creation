#!/usr/bin/env python3
"""Augment instructions with constraints while avoiding conflicts using a vLLM OpenAI-compatible API."""
...
def extract_first_user_message(messages: object) -> Optional[str]:
...
def strip_constraints_from_prompt(prompt: str, constraint_text: str | None) -> str:
    if not prompt or not constraint_text:
        return prompt
    trimmed = prompt.rstrip()
    constraint_trimmed = str(constraint_text).strip()
    if trimmed.endswith(constraint_trimmed):
        trimmed = trimmed[: -len(constraint_trimmed)].rstrip()
    return trimmed


def build_user_prompt(template: str, raw_prompt: str, other_constraints: str) -> str:
    return (
        template
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Augment instructions with constraints while avoiding conflicts using a vLLM model."
    )
    parser.add_argument(
        "--input_dataset",
        required=True,
    ...
    parser.add_argument(
        "--output_path",
        default=None,
        help="Optional JSONL path to save augmented outputs.",
    )
    parser.add_argument(
        "--save_to_disk",
        default=None,
    ...
    parser.add_argument(
        "--system_prompt_path",
        default="prompt/constraint_augmentation_conflict.txt",
        help="Path to the system prompt file.",
    )
    parser.add_argument(
        "--user_prompt_path",
        default="prompt/constraint_augmentation_conflict_user.txt",
    ...
    parser.add_argument(
        "--instruction_field",
        default="instruction",
        help="Primary field name for the raw prompt (fallback when no message field is present).",
    )
    parser.add_argument(
        "--other_constraints_fields",
        default="constraint,constraint_verifiable,constraint_text,constraints",
        help=(
            "Comma-separated field names to read existing constraints from "
    ...
    if not args.output_path and not args.save_to_disk and not args.save_prompts_jsonl:
        parser.error("Either --output_path, --save_to_disk, or --save_prompts_jsonl must be specified.")
    ...
    if "{other_constraints}" not in user_prompt_template:
        raise ValueError("User prompt template must contain {other_constraints}.")
    ...
    if args.partition_num > 1:
        ...
        print(
            f"[PARTITION] Processing partition {args.partition_index} of {args.partition_num}: "
    ...
    constraint_fields = parse_fields(args.other_constraints_fields)

    def build_messages(index: int, example: dict) -> Optional[tuple[str, str, list[dict]]]:
        """Build the messages for an example. Returns (raw_prompt, other_constraints, messages) or None."""
        other_constraints = None
        for field in constraint_fields:
            if field in example and example[field]:
                value = example[field]
                other_constraints = value if isinstance(value, str) else str(value)
                break
        if not other_constraints:
            return None

        raw_prompt = None
        if "raw_prompt" in example and example["raw_prompt"]:
            value = example["raw_prompt"]
            raw_prompt = value if isinstance(value, str) else str(value)
        else:
            candidate = None
            if "messages_verifiable" in example:
                candidate = extract_first_user_message(example["messages_verifiable"])
            if not candidate and "messages" in example:
                candidate = extract_first_user_message(example["messages"])
            if candidate:
                raw_prompt = candidate
            else:
                raw_prompt = get_original_instruction(example, args.instruction_field)

        if not raw_prompt:
            return None

        raw_prompt = strip_constraints_from_prompt(raw_prompt, other_constraints)
        if not raw_prompt:
            return None
        ...
        system_prompt = (
            system_prompt_template.replace("{reference_constraints}", sampled_constraints)
            .replace("{lower_bound}", str(args.lower_bound))
            .replace("{upper_bound}", str(args.upper_bound))
        )

        user_prompt = build_user_prompt(user_prompt_template, raw_prompt, other_constraints)
        if args.no_system:
        ...
        return raw_prompt, other_constraints, messages

    def build_result(index: int, example: dict) -> Optional[dict]:
        result = build_messages(index, example)
        if result is None:
            return None
        raw_prompt, other_constraints, messages = result
